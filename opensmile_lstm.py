...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dataset-zip",
        type=str,
        default="/mnt/e/largescaleml/dsp_final_project/M214_project_data.zip",
        help="Path to dataset zip file",
    )
    parser.add_argument(
        "--extract-dir",
        type=str,
        default="/mnt/e/largescaleml/dsp_final_project/M214_project_data",
    ...
    parser.add_argument(
        "--log-file",
        type=str,
        default="/mnt/e/largescaleml/dsp_final_project/logs/run_local.log",
    ...
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="/mnt/e/largescaleml/dsp_final_project/best_checkpoint.pt",
...
def extract_feature(audio, fs):
    ...
    try:
        ...
        raise ImportError("librosa is required for MFCC extraction.") from exc
...
def build_smile(feature_level_name: str) -> Smile:
    feature_level = (
        FeatureLevel.LowLevelDescriptors
        if feature_level_name == "lld"
        else FeatureLevel.Functionals
    )
    return Smile(feature_set=FeatureSet.eGeMAPSv02, feature_level=feature_level)
...
def select_robust_features_with_xgb(train_feat, y_train, seed=0, val_ratio=0.2):
    ...
    k_candidates = sorted(
        set(k for k in [8, 12, 16, 20, 24, 32, 48, 64] if k <= feat_dim)
    )
    if feat_dim not in k_candidates:
        k_candidates.append(feat_dim)
...
def log_opensmile_features(logger, smile: Smile, feature_level: str, combine_mfcc: bool):
...
def log_and_save_selected_features(logger, feature_names, selected_idx, output_path):
    selected_names = [feature_names[i] if i < len(feature_names) else f"feat_{i}" for i in selected_idx]
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("index,feature_name\n")
        for idx, name in zip(selected_idx, selected_names):
            f.write(f"{idx},{name}\n")
...
def build_dataloaders(
    ...
    logger.info(
        "DataLoaders ready | train_batches=%d test_clean_batches=%d",
        len(train_loader),
        len(test_loader),
    )
...
def train_model(
    ...
    if saved_checkpoint is None:
        ...
        try:
            ...
            if isinstance(loaded_meta, dict) and "history" in loaded_meta:
                history = loaded_meta["history"]
...
def main() -> int:
    ...
    logger.info(
        ...
        "device=%s feature_level=%s use_xgb_feature_selection=%s xgb_val_ratio=%.3f "
    ...
    smile = build_smile(args.feature_level)
    log_opensmile_features(logger, smile, args.feature_level, args.combine_mfcc)

    train_feat, train_label = load_dir_with_egemaps(
        train_dir, smile, combine_mfcc=args.combine_mfcc, desc="Train"
    )
    test_feat, test_label = load_dir_with_egemaps(
        test_clean_dir, smile, combine_mfcc=args.combine_mfcc, desc="Test clean"
    )
    noisy5_feat, noisy5_label = load_dir_with_egemaps(
        test_noisy_5db_dir, smile, combine_mfcc=args.combine_mfcc, desc="Test noisy 5dB"
    )
    noisy10_feat, noisy10_label = load_dir_with_egemaps(
        test_noisy_10db_dir, smile, combine_mfcc=args.combine_mfcc, desc="Test noisy 10dB"
    )
    ...
    selected_idx = np.arange(train_feat[0].shape[0], dtype=np.int64)
    feature_names = build_feature_names(smile, args.combine_mfcc, train_feat[0].shape[0])

    if args.use_xgb_feature_selection:
        if XGBClassifier is None:
            logger.warning("XGBoost not installed; skipping feature selection.")
        else:
            y_train_for_fs = np.array(train_label, dtype=np.int64)
            selected_idx = select_robust_features_with_xgb(
                train_feat,
                y_train_for_fs,
                seed=args.seed,
                val_ratio=args.xgb_val_ratio,
            )
            train_feat = apply_selected_channels(train_feat, selected_idx)
            test_feat = apply_selected_channels(test_feat, selected_idx) if test_feat else test_feat
            noisy5_feat = apply_selected_channels(noisy5_feat, selected_idx) if noisy5_feat else noisy5_feat
            noisy10_feat = apply_selected_channels(noisy10_feat, selected_idx) if noisy10_feat else noisy10_feat
            feature_names = [feature_names[i] if i < len(feature_names) else f"feat_{i}" for i in selected_idx]
            logger.info("[XGB FS] Applied selected channels. New feature dim: %d", train_feat[0].shape[0])
            selected_idx = np.arange(len(feature_names), dtype=np.int64)
    ...
    if args.save_feature_summary:
        ...
        log_and_save_selected_features(logger, feature_names, selected_idx, selected_feature_path)
    ...
    epoch_prefix = f"best_ep_{best_10db_ep}" if best_10db_ep >= 0 else f"best_ep_{args.num_epochs}"
    if args.save_cm_dir:
        os.makedirs(args.save_cm_dir, exist_ok=True)
        clean_path = os.path.join(args.save_cm_dir, f"{epoch_prefix}_cm_clean.png")
        noisy5_path = os.path.join(args.save_cm_dir, f"{epoch_prefix}_cm_noisy5db.png")
        noisy10_path = os.path.join(args.save_cm_dir, f"{epoch_prefix}_cm_noisy10db.png")
        logger.info(
            "Saving confusion matrices to %s with prefix '%s'",
            args.save_cm_dir,
            epoch_prefix,
        )
        curve_path = save_training_curves(history, args.save_cm_dir, epoch_prefix)
        if curve_path:
