...
Original file is located at
    https://colab.research.google.com/drive/16XJUSko3ojgrSDjW81U3bvd5eU6MqD0x
"""
...
def extract_feature(audio, fs):
    ...
    """
    """Frame-wise LPCC (from LPC) + CMVN. Returns (F, T)."""
    ORDER = 12          # LPC order
    N_CEPS = 13         # number of cepstral coeffs to keep (like MFCC count)
    ...
    lpcc_feats = []
    for x in frames:
        if np.allclose(x, 0, atol=1e-6) or np.var(x) < 1e-8:
            a = np.zeros(ORDER + 1, dtype=np.float32)
            a[0] = 1.0
        else:
            a = librosa.lpc(x, order=ORDER).astype(np.float32)

        c = lpc_to_lpcc(a, n_ceps=N_CEPS)   # (N_CEPS,)
        lpcc_feats.append(c)

    feats = np.stack(lpcc_feats, axis=0).T  # (N_CEPS, T)
...
def lpc_to_lpcc(a, n_ceps):
    """
    Convert LPC polynomial coefficients a (length p+1, a[0]=1)
    to LPCC (cepstral) coefficients c[0..n_ceps-1].
    """
    p = len(a) - 1
    c = np.zeros(n_ceps, dtype=np.float32)
    # c[0] often relates to gain; if you don't estimate gain, keep 0
